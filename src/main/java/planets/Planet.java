...
import planets.saturn.Enceladus;
import planets.saturn.Mimas;
...
public class Planet extends CelestialBody {
    ...
    public void createSaturnSatellite(Planet saturn, AssetManager assetManager) {
        Enceladus enceladus = new Enceladus(assetManager);
        saturn.addSatellite(enceladus);

        Mimas mimas = new Mimas(assetManager);
        saturn.addSatellite(mimas);

    }

    // Tạo vành đai cho hành tinh
    public Ring createPlanetRings(AssetManager assetManager, String texturePath1, String texturePath2, String shadowTexturePath, float innerOffset, float ringWidth, float orbitSpeed) {
        ...
        Ring ring = new Ring(this.name + "_Ring", ringMesh, ringMaterial, orbitSpeed);
